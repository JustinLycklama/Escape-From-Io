...
public class GridRayCasting {
    ...
    /// </param>
    public GridRayCasting(Func<int, int, bool> blocksLight, Action<int, int> setVisible,
    ...
    void Compute(uint octant, int originX, int originY, int rangeLimit, int x, Slope top, Slope bottom) {
        for(; (uint)x <= (uint)rangeLimit; x++) // rangeLimit < 0 || x <= rangeLimit
        {
            ...
            if(top.X == 1) {
                ...
                if(BlocksLight(x, topY, octant, originX, originY)) // if the top tile is a wall...
                {
            ...
            for(int y = topY; y >= bottomY; y--) {
                ...
                if(isOpaque &&
                   (y == topY && top.LessOrEqual(y * 2 - 1, x * 2) && !BlocksLight(x, y - 1, octant, originX, originY) ||
                    y == bottomY && bottom.GreaterOrEqual(y * 2 + 1, x * 2) && !BlocksLight(x, y + 1, octant, originX, originY))) {
                    isOpaque = false;
                }
                ...
                if(x != rangeLimit) {
                    if(isOpaque) {
                        ...
                        {                  // adjust the bottom vector upwards and continue processing it in the next column.
                            ...
                            else Compute(octant, originX, originY, rangeLimit, x + 1, top, new Slope(y * 2 + 1, x * 2));
                        }
                        wasOpaque = 1;
    ...
    bool BlocksLight(int x, int y, uint octant, int originX, int originY) {
        int nx = originX, ny = originY;
        switch(octant) {
