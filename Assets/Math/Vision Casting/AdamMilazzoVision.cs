...
public class AdamMilazzoVision {
    ...
    /// </param>
    public AdamMilazzoVision(Func<int, int, bool> blocksLight, Action<int, int> setVisible, Func<int, int, int> getDistance) {
        _blocksLight = blocksLight;
        GetDistance = getDistance;
        _setVisible = setVisible;
    }

    public void Compute(uint originX, uint originY, int rangeLimit) {
        _setVisible((int)originX, (int)originY);
    ...
    void Compute(uint octant, uint originX, uint originY, int rangeLimit, uint x, Slope top, Slope bottom) {
        ...
        for(; x <= (uint)rangeLimit; x++) // (x <= (uint)rangeLimit) == (rangeLimit < 0 || x <= rangeLimit)
        {
            ...
            {              // starts at 1/1 and remains 1/1 as long as it doesn't hit anything, so it's a common case
            ...
            } else // top < 1
              {
                ...
                if(BlocksLight(x, topY, octant, originX, originY)) // if the tile blocks light (i.e. is a wall)...
                {
                    ...
                    if(top.GreaterOrEqual(topY * 2 + 1, x * 2) && !BlocksLight(x, topY + 1, octant, originX, originY)) topY++;
                } else // the tile doesn't block light
                  {
                    ...
                    uint ax = x * 2; // center
                    if(BlocksLight(x + 1, topY + 1, octant, originX, originY)) ax++; // use bottom-right if the tile above and right is a wall
            ...
            {                 // bottom.Y starts at zero and remains zero as long as it doesn't hit anything, so it's common
            ...
            } else // bottom > 0
              {
                ...
                if(bottom.GreaterOrEqual(bottomY * 2 + 1, x * 2) && BlocksLight(x, bottomY, octant, originX, originY) &&
                   !BlocksLight(x, bottomY + 1, octant, originX, originY)) {
                    bottomY++;
                }
            }
            ...
            for(uint y = topY; (int)y >= (int)bottomY; y--) // use a signed comparison because y can wrap around when decremented
            {
                ...
                {
                    bool isOpaque = BlocksLight(x, y, octant, originX, originY);
                    ...
                    if(x != rangeLimit) // but don't bother adjusting them if this is the last column anyway
                    {
                        if(isOpaque) {
                            ...
                            {                  // so adjust the bottom vector upward and continue processing it in the next column
                                ...
                                if(BlocksLight(x, y + 1, octant, originX, originY)) nx--; // top left if the corner is not beveled
                                ...
                                {                       // created by adjusting the bottom is only valid if that's the case
                                    ...
                                    else Compute(octant, originX, originY, rangeLimit, x + 1, top, new Slope(ny, nx));
                            ...
                            if(wasOpaque > 0) // if we found a transition from opaque to clear, adjust the top vector downwards
                            {
                                ...
                                uint nx = x * 2, ny = y * 2 + 1; // the bottom of the opaque tile (oy*2-1) equals the top of this tile (y*2+1)
                                                                 // NOTE: if you're using full symmetry and want more expansive walls (recommended), comment out the next line
                                if(BlocksLight(x + 1, y + 1, octant, originX, originY)) nx++; // check the right of the opaque tile (y+1), not this one
    ...
    bool BlocksLight(uint x, uint y, uint octant, uint originX, uint originY) {
    ...
    void SetVisible(uint x, uint y, uint octant, uint originX, uint originY) {
        uint nx = originX, ny = originY;
        switch(octant) {
            case 0: nx += x; ny -= y; break;
            case 1: nx += y; ny -= x; break;
