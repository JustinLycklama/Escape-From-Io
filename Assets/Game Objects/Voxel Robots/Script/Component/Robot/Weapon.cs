namespace MoenenGames.VoxelRobot {
    ...
    /*public class Weapon : MonoBehaviour, Controllable {
        ...
        public void Animate() {
            if(ReadyToShoot) {
                prevAttackTime = Time.time + AttackFrequency;
            }
     ...
     //           PlayAllParticles();
					...
					//);
                prevAttackTime = Time.time + 0.4f; //Random.Range(0f, RandomTimeGap);
            }
            //}
        }
		...
		void ModelUpdate () {
			...
			Model.localPosition = InitLocalPos + Vector3.forward * LerpCurveF.Evaluate(AttackFrequency - t) + CurrentCurveRandom;
